...
const ElevatorMirror: React.FC<ElevatorMirrorProps> = ({
  ...
  const jointsQualityFactor = useSelector(
    (state: RootState) => state.elevator.joints?.qualityFactor ?? 1.0
  );
  ...
  const mirrorMaterialProps = useMemo(
    () => ({
      color: "#ffffff",
      blur: isHighQuality
        ? ([50, 25] as [number, number])
        : ([400, 200] as [number, number]),
  ...
  const mirrorFrameProps = useMemo(
    () => ({
      position: [0, mirror.position, -dimensions.depth / 2 + 0.04] as [
        number,
        number,
        number
      ],
      args: [mirror.width + 0.05, mirror.height + 0.05, 0.005] as [
        number,
        number,
        number
      ],
  ...
  const fullMirrorProps = useMemo(
    () => ({
      position: [0, mirror.position, -dimensions.depth / 2 + 0.05] as [
        number,
        number,
        number
      ],
  ...
  const doubleMirrorLeftProps = useMemo(
    () => ({
      ...
      args: [mirror.width / 2 - 0.05, mirror.height, 0.01] as [
        number,
        number,
        number
      ],
  ...
  const tripleMirrorLeftProps = useMemo(
    () => ({
      ...
      args: [mirror.width / 3 - 0.05, mirror.height, 0.01] as [
        number,
        number,
        number
      ],
